...
allowed-tools: Read, Write, Bash(date:*), mcp__things
---
...
First, establish today's date. Run `date "+%Y-%m-%d %A"` and use the result as the reference for everything below: today's journal filename (`YYYY_MM_DD.md`), the last-7-days window, the 3-day gap check, and the two-week task scope. Don't infer the date, read it from the system.
